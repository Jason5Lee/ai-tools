...
#[instrument(name = "load_config (command)", level = "info")]
...
#[tauri::command]
#[instrument(name = "get_config (command)", level = "info", skip(handle))]
pub fn get_config(handle: tauri::AppHandle) -> Arc<Config> {
    handle.state::<AppState>().get_config()
}

#[tauri::command]
#[instrument(name = "save_config (command)", level = "info", skip(handle, config))]
